...
pub fn scoped(scope: @EnumLiteral()) type {
    return struct {
        pub fn verbose(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Verbose, scope, format, args);
        }

        pub fn debug(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Debug, scope, format, args);
        }

        pub fn info(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Information, scope, format, args);
        }

        pub fn warn(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Warning, scope, format, args);
        }

        pub fn err(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Error, scope, format, args);
        }

        pub fn fatal(comptime src: SourceLocation, comptime format: []const u8, args: anytype) void {
            seq_zig.seqLogFn(src, .Fatal, scope, format, args);
        }
    };
}
